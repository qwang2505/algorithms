...
{
public:
    virtual const char* get_text(DomNodeKey node) const = 0;
    virtual const char* get_tag(DomNodeKey node) const = 0;
    virtual const char* get_attribute(DomNodeKey node, const char* name) const = 0;
    ...
    virtual bool drop_node(DomNodeKey node) const = 0;
    virtual int get_child_count(DomNodeKey node) const = 0;//nice to have
    virtual ~IDomNodeVisitor(){};
};

class DomNodeVisitor: public IDomNodeVisitor
{
public: 
    virtual const char* get_text(DomNodeKey node) const
    {
        return node->get_text();
    };
    virtual const char* get_tag(DomNodeKey node) const
    {
        return node->get_tag();
    };
    virtual const char* get_attribute(DomNodeKey node, const char* name) const
    {
    ...
    {
        const std::vector<DomNodeKey>* children = node->get_children();
        if (children->size() > 0 && is_html_element((*children)[0]))
        {
            return (*children)[0];
        }
        return NULL;
    ...
    virtual bool drop_node(DomNodeKey node) const
    {
        return DomNode::drop_node(node);
    };

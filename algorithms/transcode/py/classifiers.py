...
class FeatureExtractor(object):
    def __init__(self, parameters, all_features):
        '''
        init parameters:
        parameters: global feature extraction parameters
        all_features: all_features, just an empty dict now.
        '''
    ...
    def extract_feature(self, node, feature_name):
        if FeatureExtractor._extractors.has_key(feature_name):
            # extractors are all lambda functions
    ...
    def extract_features(self, node, feature_names):
        ...
        for feature_name in feature_names:
            if self._features.has_key(feature_name):
                # here use features dict, as in all_features. like a default
                # value.
        ...
        try:
            # TODO store features result in node.
...
class ClassifierBase(object):
    ...
    def __init__(self, name, parameters, config, all_features):
        ...
        parameters: global feature extraction parameters
        config: config for current classifier
        all_features: extra features? Now just an empty dict
        '''
        self._classifier_name = name
        self._parameters = parameters
        self._config = config
        self._all_features = all_features
        # get feature extractor. TODO understand this.
    ...
    def _extract_features(self, node):
        # extract node features, defined by features in config.
...
class BooleanClassifier(ClassifierBase):
    ...
    def _execute_model(cls, features, funcs, config, parameters):
        ...
        """
        # for all features. and result, all must be succeed.
